...
const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const [profile, setProfile] = useState<ProfileExtended>(CURRENT_USER);
  ...
  // Effect to load profile data based on ID
  useEffect(() => {
    if (viewingProfileId && viewingProfileId !== "me") {
      const user = MOCK_USERS.find((u) => u.id === viewingProfileId);
      if (user) {
        setProfile({
          ...CURRENT_USER,
          ...user,
  ...
  const isMe =
    !viewingProfileId ||
    viewingProfileId === "me" ||
    viewingProfileId === CURRENT_USER.id;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "accepting_guests":
        return "bg-green-100 text-green-700 border-green-200";
      case "maybe_accepting_guests":
        return "bg-yellow-100 text-yellow-700 border-yellow-200";
      case "not_accepting_guests":
        return "bg-red-100 text-red-700 border-red-200";
      case "wants_to_meet_up":
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "accepting_guests":
        return "Accepting Guests";
      case "maybe_accepting_guests":
        return "Maybe Accepting Guests";
      case "not_accepting_guests":
        return "Not Accepting Guests";
      case "wants_to_meet_up":
        return "Wants to Meet Up";
      default:
        return status;
    }
  };
  ...
  const handleUpdateImage = (type: "avatar" | "cover") => {
    const url = window.prompt(`Enter new ${type} image URL:`);
    if (url) {
      if (type === "avatar") handleInputChange("avatarUrl", url);
      else handleInputChange("coverUrl", url);
    }
  };

  const handleAddPhoto = () => {
    const url = window.prompt("Enter new photo URL:");
    if (url) {
      setProfile((prev) => ({ ...prev, photos: [...prev.photos, url] }));
    }
  };
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-full p-1 ${profile.verification.payment ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                    >
                    ...
                    <span
                      className={`font-medium text-sm ${profile.verification.payment ? "text-gray-800" : "text-gray-400"}`}
                    >
                      Payment Verified
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-full p-1 ${profile.verification.phone ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                    >
                    ...
                    <span
                      className={`font-medium text-sm ${profile.verification.phone ? "text-gray-800" : "text-gray-400"}`}
                    >
                      Phone Verified
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-full p-1 ${profile.verification.governmentId ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                    >
                    ...
                    <span
                      className={`font-medium text-sm ${profile.verification.governmentId ? "text-gray-800" : "text-gray-500"}`}
                    >
                      Government ID
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`rounded-full p-1 ${profile.verification.address === "verified" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                    >
                      {profile.verification.address === "verified" ? (
                        <Check size={12} />
                      ) : (
                        <AlertCircle size={12} />
                      )}
                    ...
                    <span
                      className={`font-medium text-sm ${profile.verification.address === "verified" ? "text-gray-800" : "text-gray-500"}`}
                    >
                      Address{" "}
                      {profile.verification.address === "pending"
                        ? "(Pending)"
                        : ""}
                    </span>
                  </div>
