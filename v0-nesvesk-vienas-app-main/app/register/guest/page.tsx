...
export default function GuestRegistrationPage() {
  ...
  const [formData, setFormData] = useState<Partial<GuestProfile> & { districtOther?: string }>({
    city: '',
    district: '',
    districtOther: '',
    groupSize: 1,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const profile: GuestProfile = {
      ...
      lookingFor: formData.lookingFor || 'games',
      description: formData.description || '',
    };

    // Save profile to localStorage
    const profiles = JSON.parse(localStorage.getItem('guestProfiles') || '[]');
    const existingIndex = profiles.findIndex((p: GuestProfile) => p.userId === user!.id);
    if (existingIndex >= 0) {
        ...
        <div className="container mx-auto max-w-3xl px-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Tell us about yourself and what you're looking for</CardDescription>
            </CardHeader>
            <CardContent>
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min="1"
                    onChange={(e) =>
                      setFormData({ ...formData, groupSize: Number.parseInt(e.target.value) })
                    }
                    required
                    type="number"
                  ...
                  <p className="text-muted-foreground text-xs">
                    How many people in your group (including you)?
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <RadioGroup
                    onValueChange={(value: any) => setFormData({ ...formData, lookingFor: value })}
                    value={formData.lookingFor}
                  >
                    {ATMOSPHERE_OPTIONS.map((option) => (
                      <div className="flex items-center space-x-2" key={option.value}>
                        <RadioGroupItem id={`looking-${option.value}`} value={option.value} />
                        <Label className="font-normal" htmlFor={`looking-${option.value}`}>
                          {t[option.key]}
                        </Label>
                      </div>
                    ))}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Tell hosts about yourself, why you're looking for a place to celebrate, any special interests..."
                    required
                    rows={5}
